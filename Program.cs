...
//Reference for Option A: https://stackoverflow.com/questions/43817351/c-sharp-proxy-authentication-using-wininet
...
namespace TallariumChallenges2020
{
    class Program
    {
        ...
        //private static InternetPerConnOptionList m_list = new InternetPerConnOptionList();  // use this to store proxy settings
        ...
        public static bool SetProxySetting(bool EnableAutoDetect)
        {
            ...
            //m_list = new InternetPerConnOptionList();
            ...
            // get standard memory size for single internet option struct
            //int optSize = Marshal.SizeOf(typeof(InternetConnectionOption));

            // Allocate memory for required internet option structs (2 at most) and get pointer to memory location
            //optionsPtr = Marshal.AllocCoTaskMem(optSize * options.Length);
            ...
            //ipcoListPtr = Marshal.AllocCoTaskMem((int)list.dwSize);
            ...
            // FREE the data in unmanaged memory  
            //FreePointerMemory(optionsPtr, ipcoListPtr);
        ...
        { 
            ...
            //Create a pointer to an integer in unmanaged memory, this will be used for storing the output size in memory value from the InternetQueryOption method
            //IntPtr sizePtr = Marshal.AllocCoTaskMem(Marshal.SizeOf(typeof(Int32)));
            //Marshal.WriteInt32(sizePtr, 0);
            
            //For monitoring and debugging
            Debug.WriteLine("iSize before1: {0}kb", Marshal.ReadInt32(sizePtr)/1024);

            // call the API method to query settings on a null buffer to get the required memory size for the result, and get result status as integer
            InternetQueryOption(
                        IntPtr.Zero,
                        InternetOption.INTERNET_OPTION_PER_CONNECTION_OPTION,
                        IntPtr.Zero, 
            ...
            else
            {
                ...
                //convert buffers to structs in managed memory to read values
                ...
                //Console.WriteLine("{0, 10} - Current Proxy Setting for 'Automatically detect proxy settings': {1:G}","STATE",  (ProxySettingState)m_iLastProxySetting);
            ...
            {  // show the error codes, they might be helpful
                ...
                //set last proxy setting to undefined
                //m_iLastProxySetting = (int)ProxySettingState.UNDEFINED;
            } 
